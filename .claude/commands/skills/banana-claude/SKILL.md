...
```bash
py ${CLAUDE_SKILL_DIR}/scripts/presets.py list
```
...
For CSV-driven batch: `py ${CLAUDE_SKILL_DIR}/scripts/batch.py --csv path/to/file.csv`
...
| MCP unavailable | Fall back to direct API: `py ${CLAUDE_SKILL_DIR}/scripts/generate.py --prompt "..." --aspect-ratio "16:9"` or `py ${CLAUDE_SKILL_DIR}/scripts/edit.py --image PATH --prompt "..."`. These call the Gemini REST API directly with no MCP dependency. |
...
```bash
py ${CLAUDE_SKILL_DIR}/scripts/cost_tracker.py log --model MODEL --resolution RES --prompt "brief description"
```
...
Run `py scripts/setup_mcp.py` to configure the MCP server. Requires:
- Node.js 18+ (npx)
- Google AI API key (free at https://aistudio.google.com/apikey)

Verify: `py scripts/validate_setup.py`
